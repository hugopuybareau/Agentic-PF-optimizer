...
class VectorStoreService:
    ...
    def store_news_items(self, news_items: list[NewsItem], asset_key: str):
        try:
            ...
            if points:
                self.client.upsert(
                    collection_name=self.news_collection_name, points=points
                )
        ...
        try:
            ...
            if asset_keys:
                if len(asset_keys) == 1:
                    filters.append(
                        FieldCondition(
                            key="asset_key", match=MatchValue(value=asset_keys[0])
                        )
                    )
                else:
                    filters.append(
                        FieldCondition(key="asset_key", match=MatchAny(any=asset_keys))
                    )
            filters.append(FieldCondition(key="stored_at", range=Range(gte=cutoff_ts)))
    ...
    def store_analysis_result(self, analysis_result: dict, portfolio_hash: str):
        try:
            ...
            self.client.upsert(
                collection_name=self.analysis_collection_name, points=[point]
            )
    ...
    def get_portfolio_history(
        self, portfolio_hash: str, days_back: int = 30
    ) -> list[dict]:
        try:
            cutoff_ts = (datetime.now() - timedelta(days=days_back)).timestamp()
            filters: list[Condition] = [
                FieldCondition(
                    key="portfolio_hash", match=MatchValue(value=portfolio_hash)
                ),
                FieldCondition(key="timestamp", range=Range(gte=cutoff_ts)),
            ]
